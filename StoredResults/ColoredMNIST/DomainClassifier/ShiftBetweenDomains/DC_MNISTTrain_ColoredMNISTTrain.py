...
log_filename = f"logs/MNISTTrainSource_ColoredMNISTTrainTarget_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def run_model(test_source_samples,test_target_samples):
    ...
    source_dataset = torchvision.datasets.MNIST(root='./data', train=True, download=True, transform=source_transform)
    ...
    target_dataset = datasets.ImageFolder(root='ColoredMNIST/train', transform=target_transform)
    ...
    def evaluate(model, source_unlabeled_indices, target_unlabeled_indices):
        ...
        log_msg = f"Evaluation Accuracy: {accuracy:.2f}%, Source: {len(source_sample_indices)}, Target: {len(target_sample_indices)}, Unlabeled Source:{len(source_unlabeled_indices)}, Unlabeled Source:{len(target_unlabeled_indices)}"
