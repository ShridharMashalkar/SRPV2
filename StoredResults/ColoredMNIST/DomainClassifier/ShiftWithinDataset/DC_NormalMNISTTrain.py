...
log_filename = f"logs/NormalMNIST_Train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
model_filename = f"models/NormalMNIST_Train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pth"
