...
log_filename = f"logs/ColoredMNIST_Test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
model_filename = f"models/ColoredMNIST_Test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pth"
...
if __name__ == '__main__':
    for i in range(10):
        ...
        train_data = datasets.ImageFolder(root='ColoredMNIST/test', transform=train_transform)
