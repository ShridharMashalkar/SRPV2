...
log_filename = f"logs/ColoredMNISTTestSource_MNISTTestTarget_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def run_model(test_source_samples,test_target_samples):
    ...
    source_dataset = datasets.ImageFolder(root='ColoredMNIST/test', transform=source_transform)
